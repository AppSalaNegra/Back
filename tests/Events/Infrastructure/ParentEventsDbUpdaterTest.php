...
class ParentEventsDbUpdaterTest extends TestCase
{
    ...
    public function testItShouldPersistData(): void
    {
        ...
        $this->assertTrue(Mockery::getContainer()->mockery_getExpectationCount() === 4);
    }

    public function testItShouldThrowExceptionWhenEncodeFailed(): void
    {
        $this->expectException(EncodeFailed::class);
        ...
        $apiHandler->shouldReceive('getParentEvents')->once()->andReturn($this->events);
        $repository->shouldReceive('findByTitle')->withArgs([$title])->once()->andReturn(null);
        $encoder->shouldReceive('parseDataToEvent')->withArgs($this->events)->once()->andThrow(EncodeFailed::class);

        $updater->persistIfNotExists();
    }

    public function testItShouldThrowExceptionWhenApiFails(): void
    {
